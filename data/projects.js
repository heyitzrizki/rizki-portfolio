const projects = [
  {
    ...
    overview:
      "This project demonstrates an end-to-end credit risk decision-support system that estimates borrower default risk, assigns credit grades, calculates expected credit loss, and simulates borrower-level and portfolio-level stress scenarios. The system is designed to connect machine learning outputs with practical credit risk workflows such as approval recommendation, portfolio monitoring, high-risk watchlist management, and expected loss analysis.",
    problem:
      "Credit risk teams need more than raw prediction scores. A useful credit rating system should produce calibrated and interpretable risk estimates, translate those estimates into business actions, and support portfolio-level risk monitoring. In real lending environments, risk decisions also require expected loss estimation and stress testing to understand how borrower risk and portfolio exposure may change under adverse conditions.",
    approach:
    ...
    systemDesign:
      "The system is designed as a layered credit risk pipeline that separates data preprocessing, temporal feature learning, model training, probability calibration, credit grade assignment, expected loss calculation, explainability, and dashboard presentation. This structure makes the system easier to interpret, maintain, and extend. It also reflects how credit risk models are often used in practice: not as standalone prediction tools, but as part of a broader decision-support and monitoring framework.",
    dataMethod:
      "The workflow uses structured borrower-level data, including application information, financial attributes, and repayment-history patterns. Static borrower features are cleaned, encoded, aligned, and combined with temporal behavior embeddings generated from historical repayment data. The final modeling layer estimates borrower default risk, while additional post-modeling layers translate the prediction into credit grades, recommendations, expected loss, and stress scenario outputs.",
    modelPipeline:
      "The modeling pipeline combines temporal feature learning through a TCN-Attention encoder with XGBoost-based risk prediction and probability calibration. The calibrated risk estimate is then mapped into an AAA–D credit rating framework and a business recommendation: approve, review, or reject. Beyond credit scoring, the system extends the pipeline into expected credit loss calculation using PD × LGD × EAD logic and scenario-based stress testing for both individual borrowers and the overall portfolio.",
    output:
      "The final output is an interactive Streamlit dashboard that includes an executive risk overview, borrower rating and stress scenario analysis, portfolio monitoring, high-risk watchlist, and ECL macro stress testing. The dashboard translates model results into business-facing outputs such as adjusted default risk, credit grade, approval recommendation, expected loss, stress loss uplift, and main risk drivers.",
    limitations:
      "The current version is a portfolio prototype and uses a historical benchmark dataset rather than live banking data. The system is batch-based and does not yet include real-time API integration. LGD is implemented as a rating-based assumption because actual recovery and write-off data are not available. Macro stress testing is scenario-based and should not be interpreted as a causal macroeconomic forecasting model. The dashboard is designed for decision support, not automated final credit approval.",
